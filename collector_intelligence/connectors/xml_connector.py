"""
XMLConnector - generic (non-RSS) XML feed. Same adapter-routing
philosophy as JSONConnector: the descriptor must say which Module 5
adapter the parsed structure maps to.
"""
...
from collector_intelligence.connector_parsing import parse_xml
...
class XMLConnector(Connector):
    ...
    def validate_config(self, source_descriptor, config):
        ...
        if not adapter_target:
            errors.append(ConnectorError(
                ...
                message="xml_connector requires 'adapter_target' - which "
                        "Module 5 adapter this XML payload should be routed to.",
                recoverable=False,
            ))
    ...
    def parse(self, fetch_result, source_descriptor):
        data = parse_xml(fetch_result.body)
